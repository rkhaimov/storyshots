...
Конфигурация для менеджера. Используется при запуске `storyshots` в режимах [UI](#) и [background](#).

---

## devices

Описывает список [устройств](#), в рамках которых запускаются истории. Возможные наименования устройств можно
...
Принимает [PreviewServer](#).

```ts
runUI({
    preview: createWebpackBundler(/* конфигурация */),
    /* ... */
})
```
